...
export const resolver = {
    ...
    getProductsByProducerId: async ({ producerId }: GetProductsByProducerIdArgs) => {
        try {
    ...
    updateProduct: async ({ _id, vintage, name, producerId }: UpdateProductArgs) => {
        try {
            const product = await Product.findById(_id)
    ...
    deleteProducts: async ({ productIds }: DeleteProductsArgs) => {
        try {
            ...
            await Promise.all(productIds.map(async (_id) => {
