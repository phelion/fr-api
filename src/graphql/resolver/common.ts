...
export const resolver = {
    ...
    createProductsWithProducer: async ({ productsWithProducer }: CreateProductsWithProducerArgs ) => {
        try {
            const result = await Promise.all(productsWithProducer.map(async (productWithProducer) => {
                const producerArgs = {
                ...
                const producer = producerQuery || await Producer.create(producerArgs)
